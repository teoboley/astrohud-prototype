...
{
    public abstract class SpatialAnchorsUtil : MonoBehaviour
    {
        ...
        public async void EnumerateAllNearbyAnchors()
        {
            ...
            {
                Debug.LogError($"{nameof(SpatialAnchorsUtil)} - Error in {nameof(EnumerateAllNearbyAnchors)}: === {ex.GetType().Name} === {ex.ToString()} === {ex.Source} === {ex.Message} {ex.StackTrace}");
